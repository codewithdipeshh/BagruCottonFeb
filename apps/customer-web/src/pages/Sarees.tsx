...
const slugToDatabaseName: Record<string, string> = {
  'kota-doria': 'Kota Doria Silk',
  'maheshwari-silk': 'Maheshwari Silk Saree',
  'cotton-mulmul': 'Mulmul Cotton Sarees',
  'handblock': 'Cotton HandBlock Sarees',
  'linen-cotton': 'Cotton Linen Saree',
  'chanderi-bagru': 'Chanderi Silk Saree',
  'khadi-cotton': 'Khadi Cotton Saree',
  'temple-border': 'Temple Border Saree',
};
...
export default function Sarees() {
  ...
  const finalProductsList = useMemo(() => {
    ...
    if (!categorySlug) return rawList;

    const targetDbName = slugToDatabaseName[categorySlug]?.toLowerCase() || categorySlug.toLowerCase();
    ...
    return rawList.filter((item: any) => {
      ...
      const cleanCatName = itemCatName.toLowerCase().trim();
      const cleanTargetName = targetDbName.trim();

      return cleanCatName === cleanTargetName || 
             cleanCatName.includes(cleanTargetName) ||
             cleanTargetName.includes(cleanCatName) ||
             itemCatSlug.toLowerCase() === categorySlug.toLowerCase() ||
             itemCatSlug.toLowerCase().replace(/-/g, '') === categorySlug.toLowerCase().replace(/-/g, '');
    });
  }, [products, categorySlug]);

  const clientFilteredProducts = useMemo(() => {
    return finalProductsList.filter((saree: any) => {
      if (inStockOnly && (saree?.quantity || 0) <= 0) return false;
      
      if (!searchQuery) return true;
      const itemTitle = saree.title || saree.name || '';
      const itemDescription = saree.description || '';
      return (
        itemTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
        itemDescription.toLowerCase().includes(searchQuery.toLowerCase())
      );
    });
  }, [finalProductsList, inStockOnly, searchQuery]);
      ...
      <section className="relative overflow-hidden bg-neutral-900 py-20 text-center">
        ...
        <div className="relative max-w-7xl mx-auto px-6 z-10">
          ...
          <h1 className="text-3xl sm:text-5xl font-serif font-light text-white mb-4 tracking-wide leading-tight">
            {categorySlug && slugToDatabaseName[categorySlug] ? slugToDatabaseName[categorySlug] : 'The Heritage Collection'}
          </h1>
          ...
          <aside className="lg:w-64 flex-shrink-0">
            <div className="bg-white border border-stone-200 rounded-2xl p-6 sticky top-36 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.02)] text-left">
              <button
                ...
                className="w-full flex items-center justify-between text-xs uppercase tracking-wider text-stone-900 font-bold border-b border-stone-100 pb-3 mb-4 focus:outline-none border-none bg-transparent cursor-pointer"
              >
          ...
          <main className="flex-1">
            <div className="bg-white border border-stone-200 rounded-2xl p-5 mb-8 flex items-center justify-between shadow-[0_4px_20px_-10px_rgba(0,0,0,0.02)]">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`p-2 rounded-lg transition-colors border-none outline-none cursor-pointer ${
            ...
            {loading ? (
              ...
              <div className="bg-white border border-stone-200 rounded-2xl p-16 text-center shadow-sm">
                <p className="text-stone-500 text-sm font-light">No drapes found matching your current selection criteria inside Database.</p>
                <button
                  type="button"
                  onClick={() => { setInStockOnly(false); }}
                  className="mt-4 text-xs font-semibold uppercase tracking-wider text-amber-800 hover:text-black transition-colors border-none bg-transparent cursor-pointer"
                >
